...
final class SwiftSQLcipherTests: XCTestCase {
    
    let dbPassword = "seKret"
    var dbFileURL: URL? = nil

    override func setUpWithError() throws {
        self.dbFileURL = mkTempFileUrl()
        addTeardownBlock() {
            try FileManager.default.removeItem(at: self.dbFileURL!)
        }
        db = try Database(path: self.dbFileURL!.absoluteString)
        try db.exec("PRAGMA key = \"\(self.dbPassword)\"")
    ...
    func testSelect(){
        ...
        let t2 = {
            let db2 = try Database(path: self.dbFileURL!.absoluteString)
            let select = try db2.statement(sql: "SELECT * FROM test ORDER BY ROWID")
            try select.step()
            db2.close()
        }
        let t3 = {
            let db3 = try Database(path: self.dbFileURL!.absoluteString)
            try db3.exec("pragma key = \"\(self.dbPassword)\"")
            let select = try db3.statement(sql: "SELECT * FROM test ORDER BY ROWID")
            var count = 0
            while (try select.step()){
                let dbl = select.double(column: 0)
                XCTAssertNotNil(dbl)
        ...
        XCTAssertNoThrow(try t())
        XCTAssertThrowsError(try t2())
        XCTAssertNoThrow(try t3())
    }
